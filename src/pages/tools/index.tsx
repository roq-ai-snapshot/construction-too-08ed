...
export function ToolListPage(props: ToolListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getTools({
        relations: ['company', 'rental_request.count'],
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('company', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ...
      : null,
    hasAccess('rental_request', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'rental_request',
          header: 'Rental Request',
          accessorKey: 'rental_request',
          cell: ({ row: { original: record } }: any) => record?._count?.rental_request,
        }
      : null,
